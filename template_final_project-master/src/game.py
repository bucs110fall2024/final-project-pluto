...
class Game:
    ...
    def __init__(self, size_x, size_y, ticks_per_bullet = 10):
        ...
        self.player = Player(self.size_x // 2, self.size_y // 2, self.size_x, self.size_y)
    ...
    def tick(self):
        ...
        self.bullets = [self.bullets[i] for i in self.field.collidelistall(bullet_hitboxes)]
        
        if(self.player.alive and self.player.hitbox().collidelist(bullet_hitboxes)!= -1):
    ...
    def random_bullet(self):
        ...
        return Projectile(position_x, position_y, speed_x, speed_y)

    def draw_score(self, surface):
        text = self.font.render('{}'.format(self.score), True, pygame.Color('white'))
        surface.blit(text, (10,10))
        
    def draw_newgame_message(self, surface):
        text = self.font.render('Press SPACE for new game', True, pygame.Color('white'))
        surface.blit(text, (10, self.size_y - text.get_height()-10))
        
    def death_explosion(x, y):
        return list([Projectile(x, y, xs, ys, pygame.Color('white')) for xs, ys 
                    in permutations(list(range(-10, 10)) ,2) if(xs, ys) != (0,0)])